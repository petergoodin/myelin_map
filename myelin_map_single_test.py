from myelin_map_test_funcs import dcm_convert, plot_mask_dist, plot_ants_warp, ants_reg, mask_transform, ants_rigid, subj2mni, bias_corr, image_calibration, create_mm_func, image_smooth
import numpy as np
...
#plot_ants_warp(fixed = t1_subj_nii, moving = brain_subj_mask, nslices = 5, output_name = os.path.join(working_subj_dir, 'brain_mask'))

rigid_output = ants_rigid(fixed = t1_subj_nii, moving = t2_subj_nii, output_dir = working_subj_dir)

t1_subj_bias_corr, t2_subj_bias_corr = bias_corr([t1_subj_nii, rigid_output['warped_image']], output_dir = working_subj_dir)

t1_cal, t2_cal = image_calibration(t1_subj_bias_corr = t1_subj_bias_corr, 
                                                       t2_subj_bias_corr = t2_subj_bias_corr, 
